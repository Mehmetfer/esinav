...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_id'])) {
    ...
    $st = $pdo->prepare('UPDATE src_sorular SET aktif = IF(aktif = 1, 0, 1) WHERE id = ?');
    $st->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    $row = $pdo->prepare('SELECT gorsel FROM src_sorular WHERE id = ? LIMIT 1');
    $row->execute([$id]);
    $old = $row->fetch();
    if ($old) {
        $gorsel = trim((string)($old['gorsel'] ?? ''));
        if ($gorsel !== '' && str_starts_with($gorsel, 'upload/')) {
            $fs = dirname(__DIR__) . '/assets/img/sorular/' . ltrim(str_replace('\\', '/', $gorsel), '/');
            if (is_file($fs)) @unlink($fs);
        }
        $pdo->prepare('DELETE FROM src_sorular WHERE id = ?')->execute([$id]);
...
$params = [];
if ($q !== '') {
    $where[] = '(soru LIKE ? OR secenek_a LIKE ? OR secenek_b LIKE ? OR secenek_c LIKE ? OR secenek_d LIKE ?)';
    $like = '%' . $q . '%';
    array_push($params, $like, $like, $like, $like, $like);
}
if ($ders !== '' && isset(src_ders_adlari()[$ders])) {
    $where[] = 'ders = ?';
    $params[] = $ders;
}
if ($kaynak !== '') {
...
$countSt = $pdo->prepare("SELECT COUNT(*) FROM src_sorular WHERE {$sqlWhere}");
...
$listSt = $pdo->prepare(
    "SELECT id, ders, soru, dogru, aktif, kaynak, gorsel
     FROM src_sorular WHERE {$sqlWhere}
...
?>
  <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:14px">
    <h1 style="margin:0;color:var(--navy);font-size:1.25rem">SRC Soru Havuzu <span style="color:var(--muted);font-weight:600">(<?= $total ?>)</span></h1>
    <a class="k-start-btn" href="/admin/src-soru-duzenle.php" style="padding:10px 18px;font-size:.9rem;text-decoration:none;display:inline-block;background:#1e3a8a;color:#fff;border-radius:8px;font-weight:700">+ Yeni Soru</a>
  </div>
    ...
    <form method="get" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr auto;gap:10px;align-items:end">
      ...
      <div>
        ...
        <select name="ders" style="width:100%;padding:9px 10px;border:1px solid #d1d5db;border-radius:6px">
          ...
          <?php foreach ($dersAd as $k => $v): ?>
            <option value="<?= e($k) ?>" <?= $ders === $k ? 'selected' : '' ?>><?= e($v) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div>
  ...
  <div class="card" style="padding:0;overflow:auto">
    <table style="width:100%;border-collapse:collapse;font-size:.88rem">
      ...
      <tbody>
      <?php if (!$rows): ?>
        <tr><td colspan="7" style="padding:20px;color:var(--muted)">Kayıt yok.</td></tr>
      ...
      ?>
        <tr style="border-top:1px solid #eef2f7">
          ...
          <td style="padding:10px 12px;white-space:nowrap">
            <a href="/admin/src-soru-duzenle.php?id=<?= (int)$r['id'] ?>" style="color:#1d4ed8;font-weight:700;text-decoration:none;margin-right:8px">Düzenle</a>
