...
$chipHref = static function (int $catId, string $q): string {
    ...
    return $params === [] ? 'trafik-isaretleri.php' : 'trafik-isaretleri.php?' . http_build_query($params);
};
?>
  ...
  <form class="k-sign-toolbar" method="get" action="trafik-isaretleri.php">
    <?php if ($cat > 0): ?>
    ...
    <?php if ($pages > 1): ?>
      <nav class="k-pager k-sign-pager" aria-label="Sayfa">
        ...
        $qs = static function (int $p) use ($q, $cat): string {
            ...
            return 'trafik-isaretleri.php?' . http_build_query($params);
        };
        ?>
